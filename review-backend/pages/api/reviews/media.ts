      ...
      try {
        ...
        if (files.images) {
          ...
          for (const imgFile of imageFiles) {
            const imgUrl = await saveFile(imgFile);
            if (imgUrl) {
        ...
        if (files.videos) {
          ...
          for (const vidFile of videoFiles) {
            const vidUrl = await saveFile(vidFile);
            if (vidUrl) {
        ...
        const existingImageUrls = review.imageUrl ? JSON.parse(review.imageUrl) : [];
        const existingVideoUrls = review.videoUrl ? JSON.parse(review.videoUrl) : [];

        // Combine existing and new media URLs
        const combinedImageUrls = [...existingImageUrls, ...imageUrls];
        const combinedVideoUrls = [...existingVideoUrls, ...videoUrls];

        // Update the review with media URLs
        const updatedReview = await prisma.review.update({
          where: { id: reviewId },
          data: {
            imageUrl: combinedImageUrls.length > 0 ? JSON.stringify(combinedImageUrls) : review.imageUrl,
            videoUrl: combinedVideoUrls.length > 0 ? JSON.stringify(combinedVideoUrls) : review.videoUrl,
          },
        });
